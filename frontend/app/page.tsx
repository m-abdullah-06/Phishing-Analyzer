...
type MimeAnalysis = {
  multipart: boolean;
  statuses: { status: "ok" | "warning"; message: string }[];
  anomalies: string[];
  risk_contribution: number;
  detail: string;
};
type AttachmentAnalysis = {
  ...
  mime_type: string;
  size_bytes: number;
  dangerous_extension: boolean;
  archive: boolean;
  nested_archive: boolean;
  executable: boolean;
  macro_enabled: boolean;
  suspicious_double_extension: boolean;
  vt_malicious?: number;
  mb_found?: boolean;
  mb_family?: string | null;
};
type ContentAnalysis = {
  findings: { type: string; risk_contribution: number }[];
  normal_patterns: string[];
  risk_contribution: number;
  high_pressure_credential: boolean;
  assessment: string;
};
type EspAnalysis = {
  providers: string[];
  ...
  multipart_html: boolean;
  auth_aligned: boolean;
  sender_consistent: boolean;
  normal_behavior: boolean;
  strong_legitimacy: boolean;
  risk_contribution: number;
  assessment: string;
  evidence: string[];
};
...
const SIGNAL_DESCRIPTIONS: Record<string, string> = {
  domain_spoofing:
    "The sender's domain visually mimics a known brand (homograph attack) to deceive the recipient.",
  brand_impersonation:
    "The display name or sender address attempts to impersonate a known entity without authentication alignment.",
  malicious_url:
    "One or more URLs in the email are flagged as malicious by threat intelligence feeds.",
  suspicious_url:
    "URLs use suspicious TLDs, URL shorteners, or exhibit evasive characteristics.",
  credential_harvesting:
    "The email content uses language strongly associated with credential theft (e.g., urgent password resets).",
  malicious_attachment:
    "An attachment has a known malicious hash identified by threat intelligence.",
  suspicious_attachment:
    "An attachment uses a dangerous extension, double extension, or contains macros/executables.",
  mime_anomaly:
    "The email structure contains malformed MIME boundaries or evasion techniques.",
  spf_fail:
    "The sender's IP is not authorized to send emails on behalf of the domain (SPF Hard Fail).",
  dkim_fail:
    "The cryptographic signature of the email is invalid or has been tampered with.",
  dmarc_fail:
    "The email fails DMARC alignment, meaning the visible sender does not match the authenticated identity.",
  suspicious_origin_ip:
    "The originating IP has a poor reputation or is associated with spam/malware activity.",
  high_pressure_tactic:
    "The email content creates a false sense of urgency or threatens negative consequences.",
  freemail_sender:
    "The email originates from a free email provider (e.g., Gmail, Yahoo) but claims to be official.",
};
...
const getCategory = (signal: string) => {
  ...
  if (
    signal.includes("spf") ||
    signal.includes("dkim") ||
    signal.includes("dmarc")
  )
    return "Authentication";
  if (
    signal.includes("spoofing") ||
    signal.includes("impersonation") ||
    signal.includes("sender")
  )
...
const getSeverity = (weight: number) => {
  ...
  if (weight >= 10)
    return { label: "HIGH", color: "var(--amber)", bg: "var(--amber-dim)" };
  if (weight >= 5)
    return { label: "MEDIUM", color: "var(--blue)", bg: "var(--blue-dim)" };
...
function RiskBreakdown({
  ...
  signals: { signal: string; weight: number; applied: boolean }[];
}) {
          ...
          .map(([cat, data]) => (
            ...
            >
              <div className="row-between">
                ...
                <span
                  className="mono"
                  style={{ fontSize: 12, color: "var(--red)", fontWeight: 600 }}
                >
                  +{data.total}
                </span>
              </div>
    ...
    <div className="panel">
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {sorted.map((s) => {
          ...
          const desc =
            SIGNAL_DESCRIPTIONS[s.signal] ||
            `Signal: ${s.signal.replaceAll("_", " ")}`;
          return (
            <div
              key={s.signal}
              style={{
            ...
            >
              <div style={{ padding: "12px 16px" }}>
                <div className="row-between" style={{ marginBottom: 8 }}>
                  ...
                  <span
                    className="mono"
                    style={{ fontSize: 12, color: sev.color, fontWeight: 600 }}
                  >
                    +{s.weight}
                  </span>
                </div>
      ...
      >
        <div className="row-between">
          ...
          <span
            style={{
              color:
                auth.impact.risk_contribution > 0
                  ? "var(--red)"
                  : "var(--green)",
            }}
          >
            +{auth.impact.risk_contribution}
          </span>
        </div>
        <div className="row-between">
          <span>AUTH DOMAIN</span>
          <span style={{ color: "var(--text)" }}>
            {auth.alignment.domain || "—"}
          </span>
        </div>
        <div className="row-between">
          <span>VISIBLE FROM</span>
          <span style={{ color: "var(--text)" }}>
            {auth.alignment.from_domain || "—"}
          </span>
        </div>
        <div className="row-between">
          <span>ALIGNMENT</span>
          <span
            style={{
              color: auth.alignment.aligned ? "var(--green)" : "var(--text-2)",
            }}
          >
            {auth.alignment.aligned ? "ALIGNED" : "NOT ALIGNED"}
          </span>
        </div>
      </div>
    </div>
...
export default function Home() {
  ...
  const copyReport = () => {
    ...
    const lines = [
      ...
      ...(result.sender_identity.findings.length
        ? result.sender_identity.findings.map(
            (finding) =>
              `[WARNING] ${finding.detail} (+${finding.risk_contribution})`,
          )
        : ["No sender-identity anomalies detected."]),
      "",
      "[ MIME STATUS ]",
      ...result.mime_analysis.statuses.map(
        (status) =>
          `${status.status === "ok" ? "[OK]" : "[WARNING]"} ${status.message}`,
      ),
      ...
      ...result.content_analysis.findings.map(
        (finding) =>
          `${finding.type.replaceAll("_", " ")}: +${finding.risk_contribution}`,
      ),
      ...(result.content_analysis.normal_patterns.length
        ? [
            `Normal context: ${result.content_analysis.normal_patterns.join(", ")}`,
          ]
        : []),
      "",
      "[ LEGITIMATE ESP CONTEXT ]",
      result.esp_analysis.assessment,
      ...(result.esp_analysis.evidence.length
        ? result.esp_analysis.evidence
        : ["No corroborating ESP evidence."]),
      "",
    ...
    if (result.homograph.suspicious) {
      lines.push(
        "[ DOMAIN SPOOFING ]",
        `[WARNING] ${result.homograph.detail}`,
        "",
      );
    }
    ...
    if (result.urls.length) {
      ...
      result.urls.forEach((u) => {
        lines.push(
          `${u.defanged}  [VT: ${u.vt_malicious || 0}/${u.vt_total || 0}]${u.is_shortener ? " [SHORTENER]" : ""}${u.suspicious_tld ? " [SUSPICIOUS TLD]" : ""}`,
        );
      });
      lines.push("");
    }
    ...
    if (result.attachments.length) {
      ...
      result.attachments.forEach((a) => {
        lines.push(
          `${a.filename} — ${a.mime_type} — ${a.size_bytes} bytes — SHA256: ${a.sha256}`,
        );
        lines.push(
          `  VT: ${a.vt_malicious || 0} malicious | MalwareBazaar: ${a.mb_found ? `FOUND (${a.mb_family})` : "not found"}${a.executable ? " [EXECUTABLE]" : ""}${a.suspicious_double_extension ? " [DOUBLE EXTENSION]" : ""}${a.macro_enabled ? " [MACRO-ENABLED]" : ""}${a.archive ? " [ARCHIVE]" : ""}${a.nested_archive ? " [NESTED ARCHIVE]" : ""}`,
        );
      });
      lines.push("");
    }
    ...
    if (result.origin_ip.ip) {
      lines.push(
        "[ ORIGIN IP ]",
        `${result.origin_ip.ip} — VT: ${result.origin_ip.vt_malicious} malicious, AbuseIPDB confidence: ${result.origin_ip.abuse_confidence}%`,
        "",
      );
    }

    lines.push("[ MITRE ATT&CK MAPPING ]");
    result.mitre_attack.forEach((t) => {
      const evidence =
        Array.isArray((t as any).evidence) && (t as any).evidence.length
          ? ` (evidence: ${(t as any).evidence.join(", ")})`
          : "";
      lines.push(`${t.id} — ${t.name}${evidence}`);
    });
  ...
  return (
    <main className="animate-in" style={{ paddingTop: 60 }}>
      ...
      >
        ...
        >
          Upload a raw .eml file or paste the source. Get enterprise-grade
          forensics, IOC enrichment, and automated incident reports.
        </p>
      </div>
      ...
      <div style={{ maxWidth: 680, margin: "0 auto 40px" }}>
        ...
        {mode === "upload" ? (
          <div
            ...
            style={{
              ...
              textAlign: "center",
              cursor: "pointer",
              background: fileObj ? "var(--green-dim)" : "var(--surface)",
        ...
        <button
          onClick={analyze}
          disabled={
            loading || (mode === "upload" ? !fileObj : !rawEmail.trim())
          }
          style={{
            width: "100%",
            marginTop: 16,
        ...
        >
          ...
          <div className="grid-3">
            <AuthBadge
              ...
              auth={result.authentication.spf}
              icon={Server}
            />
            <AuthBadge
              label="DKIM VERIFICATION"
              auth={result.authentication.dkim}
              icon={FileKey}
            />
            <AuthBadge
              label="DMARC VERIFICATION"
              auth={result.authentication.dmarc}
              icon={Shield}
            />
          </div>
          ...
          <div className="grid-2">
            <div className="panel">
              ...
              >
                <div className="row">
                  <span style={{ color: "var(--text-3)", width: 90 }}>
                    From:
                  </span>{" "}
                  <span style={{ color: "var(--text)" }}>
                    {result.headers.from.display_name} &lt;
                    {result.headers.from.email}&gt;
                  </span>
                </div>
                ...
                {result.headers.return_path && (
                  <div className="row">
                    ...
                    <span style={{ color: "var(--text)" }}>
                      {result.headers.return_path.email}
                    </span>
                  </div>
                )}
              </div>
            ...
            <div
              className="panel"
              style={{
                border: result.sender_identity.suspicious
            ...
            >
              <div className="row-between" style={{ marginBottom: 16 }}>
                ...
                <span
                  ...
                  style={{
                    fontSize: 11,
                    color: result.sender_identity.suspicious
                      ? "var(--red)"
                      : "var(--green)",
                    fontWeight: 600,
                  }}
                  ...
                  {result.sender_identity.findings.map((f, i) => (
                    ...
                    >
                      <div className="row-between" style={{ marginBottom: 4 }}>
                        ...
                        >
                          {f.code.replaceAll("_", " ").toUpperCase()}
                        </span>
                        <span
                          className="mono"
                          style={{
              ...
              <div className="row-between" style={{ marginBottom: 16 }}>
                <div className="row">
                  ...
                  <span className="section-label">
                    URL ANALYSIS ({result.urls.length})
                  </span>
                </div>
              </div>
                ...
                {result.urls.map((u, i) => (
                  ...
                  >
                    ...
                    <div className="row" style={{ flexShrink: 0 }}>
                      {u.is_shortener && (
                        <Tag color="var(--amber)">SHORTENER</Tag>
                      )}
                      {u.suspicious_tld && (
                        <Tag color="var(--amber)">SUS TLD</Tag>
                      )}
                      {(u.vt_malicious || 0) > 0 && (
                        <Tag color="var(--red)">
                          {u.vt_malicious}/{u.vt_total} VT
                        </Tag>
                      )}
                    </div>
                  </div>
                ...
                {result.attachments.map((a, i) => (
                  <div
                    ...
                    style={{
                      ...
                      borderRadius: "var(--r-md)",
                      border:
                        a.executable || a.suspicious_double_extension
                          ? "1px solid var(--red-border)"
                  ...
                  >
                    <div className="row-between" style={{ marginBottom: 8 }}>
                      ...
                      {a.dangerous_extension && (
                        <Tag color="var(--red)">DANGEROUS EXT</Tag>
                      )}
                    </div>
                    <div
                      className="mono"
                    ...
                    >
                      TYPE: {a.mime_type} · EXT: {a.extension || "none"} · SIZE:{" "}
                      {a.size_bytes.toLocaleString()} bytes
                    </div>
                    <div
                      className="mono"
                      style={{
                    ...
                    <div className="row" style={{ flexWrap: "wrap" }}>
                      ...
                      {a.suspicious_double_extension && (
                        <Tag color="var(--red)">DOUBLE EXT</Tag>
                      )}
                      {a.macro_enabled && (
                        <Tag color="var(--amber)">MACRO-ENABLED</Tag>
                      )}
                      {a.archive && <Tag color="var(--amber)">ARCHIVE</Tag>}
                      {a.nested_archive && (
                        <Tag color="var(--red)">NESTED ARCHIVE</Tag>
                      )}
                      {(a.vt_malicious || 0) > 0 && (
                        <Tag color="var(--red)">
                          VT: {a.vt_malicious} malicious
                        </Tag>
                      )}
                      {a.mb_found && (
                        <Tag color="var(--red)">
                          MalwareBazaar: {a.mb_family}
                        </Tag>
                      )}
                    </div>
                  </div>
          ...
          {result.mitre_attack && result.mitre_attack.length > 0 && (
            <div className="panel">
              <div className="row-between" style={{ marginBottom: 16 }}>
                <div className="row">
                  <Target size={14} color="var(--text-3)" />
                  <span className="section-label">MITRE ATT&CK MAPPING</span>
                </div>
              </div>
              ...
              <div
                style={{ display: "flex", flexDirection: "column", gap: 10 }}
              >
                {result.mitre_attack.map((technique, index) => {
                  const evidence = Array.isArray((technique as any).evidence)
                    ? (technique as any).evidence
                    : [];
                    ...
                    >
                      <div
                        style={{
                          ...
                          alignItems: "center",
                          gap: 8,
                          marginBottom: evidence.length ? 8 : 0,
                        }}
                      >
                        <span
                          className="mono"
